...
- **Строки:** 1-154
- **Функции:**
  - Инициализация тестовых матриц (строки 14-30) - квадратные и неквадратные
  - Цикл по тестовым матрицам (строки 52-130)
  - Измерение времени CPU (строки 60-67)
  - Измерение времени GPU naive (строки 70-80)
  - Измерение времени GPU shared (строки 83-93)
  - Сохранение в CSV с форматом rows,cols,... (строки 116-123)

### 5. Графики

**Данные:**
- **Файл:** `results.csv`
- **Формат:** CSV с колонками: rows,cols,cpu_time_ms,gpu_naive_time_ms,gpu_shared_time_ms,speedup_naive,speedup_shared,correct
- **Особенности:** Отдельные колонки для rows и cols, поддержка неквадратных матриц
...
- Анализ ускорения для разных размеров (квадратные и неквадратные)
