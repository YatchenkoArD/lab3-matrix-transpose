...
**Файл:** `main.c` (154 строки)

**Основные функции:**
- Инициализация тестовых данных (квадратные и неквадратные матрицы)
- Запуск тестов для различных размеров:
  - Квадратные: 256×256, 512×512, 1024×1024, 2048×2048, 4096×4096, 8192×8192
  - Неквадратные: 256×512, 512×256, 1024×2048, 2048×1024, 512×1024, 1024×512
- Измерение времени выполнения (10 прогонов для усреднения)
- Сравнение результатов CPU и GPU (две версии GPU: naive и shared memory)
- Сохранение результатов в CSV с отдельными колонками для rows и cols
...
4. **Результаты экспериментов**
   - График зависимости времени от размера матрицы (квадратные и неквадратные)
   - Таблица результатов (из results.csv)
   - Анализ ускорения для разных типов матриц
   - Сравнение производительности для квадратных и неквадратных матриц
...
- **Скрипт построения графика:** `plot_results.py` (поддержка квадратных и неквадратных матриц)
- **Результаты измерений:** `results.csv` (формат: rows,cols,cpu_time_ms,gpu_naive_time_ms,gpu_shared_time_ms,speedup_naive,speedup_shared,correct)
- **Документация по компиляции:** `COMPILE_GUIDE.md`
- **Пример CSV:** `results_example.csv`
