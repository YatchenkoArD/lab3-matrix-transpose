...
## 6. Ключевые моменты для отчета

1. **Naive подход:**
   - Простой и понятный
   - Прямой доступ к глобальной памяти
   - Проблема: некоалесцированная запись

2. **Shared Memory подход:**
   - Использует быструю локальную память
   - Коалесцированный доступ при чтении и записи
   - Padding предотвращает bank conflicts
   - Значительно быстрее для больших матриц

3. **Вычисление блоков:**
   - Округление вверх: `(N + TILE - 1) / TILE`
   - Гарантирует обработку всех элементов
   - Может создавать "лишние" потоки (проверка границ в коде)

4. **Оптимизации:**
   - Коалесцированный доступ
   - Shared memory вместо глобальной
   - Padding для избежания bank conflicts
   - Меньше потоков на блок (256 vs 1024) = больше блоков = лучше утилизация GPU
